...
後続の[可変コンテナと共有paint](variable-container-paints.md)では、前者を選び、明示した直線帯域の形状証明と別のresize policyを追加した。本資料の固定container APIと当時の評価結果は維持し、可変containerは新しい外側の契約として扱っている。
