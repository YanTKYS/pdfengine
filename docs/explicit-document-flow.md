...
この資料で記録したFCCの短文化拒否については、後続の[輪郭による衝突証明](ink-collision-certificates.md)で、同じ原本・関係・固定領域のまま8段階編集が成立した。元の拒否結果は上記の導入時評価として残す。新しい許可の根拠は、明示relationとは独立したsource glyphの幾何証明である。
