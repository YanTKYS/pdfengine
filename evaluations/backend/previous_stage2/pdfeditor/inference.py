...
The heuristics deliberately keep the observed width: a PDF does not tell us the
author's intended text-frame width. Unicode word boundaries are UAX #29 segments,
...
def _make_line(original: list[Glyph], direction: tuple[float, float] = (1.0, 0.0)) -> Line:
    glyphs: list[Glyph] = []
    for glyph in original:
        if glyphs and glyphs[-1].text and glyph.text:
            previous = glyphs[-1]
...
def _horizontal_lines(glyphs: list[Glyph]) -> list[Line]:
    ...
    for group in baselines:
        ...
        for glyph in group:
            ...
            right_edge = max(right_edge if right_edge is not None else glyph.bbox.x1,
                             glyph.bbox.x1)
        lines.extend(_make_line(piece) for piece in pieces)
