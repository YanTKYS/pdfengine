...
def _validate_written_positions(page, layout, metrics, first_seqno: int) -> None:
    """Verify emitted geometry, independently of the font's measurement API.

    Some embedded fonts measure correctly but acquire inconsistent CID /W
    entries from TextWriter. Unicode extraction alone still succeeds in that
    case. Only inspect the appended paint operations, identified by bboxlog's
    sequence numbers, so adjacent original text cannot satisfy this check.
    """
    expected = []
    for line in layout.lines:
        x = line.x
    ...
    actual = [char for span in page.get_texttrace() if span["seqno"] >= first_seqno
              for char in span["chars"]]
    # /W values are quantized to PDF font units; use the same tolerance as
    # source-font metric validation, while bounding cumulative origin error.
    tolerance = max(.12, metrics.size * .025)
    if len(actual) != len(expected) or any(
            char[0] != codepoint or abs(char[2][0] - x) > tolerance
            or abs(char[2][1] - y) > tolerance
            for char, (codepoint, x, y) in zip(actual, expected)):
        raise PdfError("writer glyph positions differ from the planned layout; "
                       "font encoding/width reconstruction is required; no output saved")
...
def replace_region(document: pymupdf.Document, page_number: int, model: PageModel,
    ...
    if any(line.text for line in layout.lines):
        first_seqno = len(page.get_bboxlog())
        writer.write_text(page, color=box.glyphs[0].style.color, overlay=True)
        _validate_written_positions(page, layout, metrics, first_seqno)
